...
pub fn parse_query_parameters(params: &HashMap<String, String>) -> Result<QueryParameters, StatusCode> {
...
fn parse_time(params: &HashMap<String, String>, key: &str) -> Result<Option<i64>, StatusCode> {
    ...
    if let Some(latest_string) = text {
        ...
        if latest_result.is_err() {
            debug!("Error parsing date: {:?} and input: {}", latest_result, latest_string);
            return Err(StatusCode::BAD_REQUEST);
        }
